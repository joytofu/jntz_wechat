...
$resultArray = array();
	...
	try {
		...
		if (!isset($html_analysis)){
            ...
            $fundtitle="          巨牛汇聪期货1号";
            //$fundprofit="累计净利润:".trim($html_analysis->find('table tbody tr td', 64)->plaintext);
            //$fundrate="参考收益率:".trim($html_analysis->find('table tbody tr td', 67)->plaintext);
            $fundCreateTime="成立日期:2015年3月16日";
            $fundvalue="单位净值:".trim($html_analysis->find('table tbody tr td', 68)->plaintext);
            $fundinfo=$fundtitle."\n".$fundCreateTime."\n".$fundvalue;
