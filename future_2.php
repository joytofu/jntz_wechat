...
$resultArray = array();
	...
	try {
		...
		if (!isset($html_analysis)){
            ...
            $fundtitle="          巨牛天子期货2号";
            //$fundprofit="累计净利润:".trim($html_analysis->find('table tbody tr td', 40)->plaintext);
            //$fundrate="参考收益率:".trim($html_analysis->find('table tbody tr td', 43)->plaintext);
            $fundCreateTime="成立日期:2015年3月16日";
            $fundvalue="单位净值:".trim($html_analysis->find('table tbody tr td', 44)->plaintext);
            $fundinfo=$fundtitle."\n".$fundCreateTime."\n".$fundvalue;
